      ...
      try {
        ...
        if (!user) {
          user = await UserManager.createOne({
            ...
            first_Name: profile.name.givenName ? profile.name.givenName : 'Nombre no proporcionado',
            last_Name: profile.name.familyName ? profile.name.familyName : 'Apellido no proporcionado',
            fromGoogle: true,
          });
        } else {
          if (!user.fromGoogle) {
